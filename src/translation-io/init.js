...
var _default = (config) => {
  if (
    ...
    config.targetLocales === undefined
  ) {
    throw new Error(`Invalid init config ${JSON.stringify(config)}`);
  }

  log.info('Running `init`.');
  log.info('Loading translations from disk...');
  const translations = (0, _extract.load)(config);
  ...
  req.post('/init', params, config).then(res => {
