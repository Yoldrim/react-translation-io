...
const attachParams = (config, data) => _objectSpread({
  client: 'rails',
  version: '1.14',
...
instance.interceptors.response.use(res => {
  ...
  // log.error(err);
  // log.error(`Error with request to \`${err && err.config && err.config.url}\``);
  console.log(`Request error to ${err && err.config && err.config.url}`)
...
const post = (url, data, config) => {
  const params = attachParams(config, data);
  return instance.request({
    url: `/projects/${config.apiKey}${url}?${_querystring.default.stringify(params)}`,
    method: 'post'
  });
}
...
const get = (url, config) => {
  const params = attachParams(config, {});
  return instance.get(`/projects/${config.apiKey}${url}`, _querystring.default.stringify(params));
}
