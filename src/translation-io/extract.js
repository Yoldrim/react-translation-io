...
var _default = (config) => {
  if (config === undefined) {
    throw new Error(`Invalid extract config ${JSON.stringify(config)}`);
  }

  log.info('Running `extract`.');
  log.info('Extracting translations...');
  const translations = JSON.stringify(load(config), null, 2);
